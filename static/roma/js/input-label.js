...
var InputLabel = {
    focus: function() {
        ...
        if (t.hasClass('inputLabelPassword'))
            InputLabel._setInputType(t.removeClass('inputLabelPassword'), 
                'password').focus().bind('blur.InputLabel', InputLabel.blur);
    },

    blur: function(e) {
        var t = jQuery(this);
        if (t.is(':password[value=""]')) {
            t = InputLabel._setInputType(this, 'text')
                .addClass('inputLabelPassword')
                .bind('focus.InputLabel', InputLabel.focus);
    ...
    submit: function() {
        jQuery('input[title].inputLabelActive').trigger('focus.InputLabel');
    },
...
(function($) { $(function() {
    $('form:has(input[title].inputLabel)').submit(InputLabel.submit);
    $('input[title].inputLabel')
        .bind('focus.InputLabel', InputLabel.focus)
        .bind('blur.InputLabel', InputLabel.blur)
        .trigger('blur.InputLabel'); // Apply the title
}); })(jQuery);
